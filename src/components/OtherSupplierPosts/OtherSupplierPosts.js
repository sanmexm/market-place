...
import './otherSupplierPosts.css'

const OtherSupplierPosts = ({userId, id, page, actionGet, postType, category, tag}) => {
    const dispatch                                          = useDispatch();
    const [isLoading, setIsLoading]                         = useState(true);
    const [allPosts, setAllPosts]                           = useState([]);
    const [percentageDifferences, setPercentageDifferences] = useState([]);
    const moreFromSupplier                                  = "More from Supplier"
    const linkTo                                            = `/post-items/${moreFromSupplier}/filtered?postType=${postType}&category=${category}&tag=${tag}`

  useEffect(() => {
    setIsLoading(true);
    if (typeof actionGet === 'function') {
      dispatch(actionGet(userId, id, page))
        .then((response) => {
  ...
  }, [userId, id, page, actionGet, dispatch]);
    ...
    <>
      <section className='items-wrapper'>
        <div className='items-header'>
          <span className='item-title'><PostTitle title={moreFromSupplier} /></span>
...
export default OtherSupplierPosts