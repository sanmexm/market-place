...
import { startLoading, endLoading, setStores, setStoresByUser, setSingleStore, setStoresSearch, setUpdateStore, setDeleteStore, setSingleUserStore, setStoreItemsById } from '../reducers/storesSlice'
//action creators
...
export const actionFetchUserStoreItems = (id) => async(dispatch) => {
    try{
        dispatch({ type: startLoading.type })
        const { data } = await api.fetchUserStoreItems(id)
        dispatch({
            type: setStoreItemsById.type,
            payload: data
        })
        dispatch({ type: endLoading.type })
        return data
...
// export const actionFetchUserStoreItems = (id, page) => async(dispatch) => {
//     try{
//         dispatch({ type: startLoading.type })
//         const { data } = await api.fetchUserStoreItems(id, page)
//         dispatch({
//             type: setStoreItemsById.type,
//             payload: data
//         })
...
export const actionFetchStoresByUser = (userId, page) => async(dispatch) => {
    try{
        dispatch({ type: startLoading.type })
        const { data } = await api.fetchStoresByUser(userId, page)
        dispatch({
...
export const actionFetchUserStore = (userId) => async(dispatch) => {
    try{
        dispatch({ type: startLoading.type })
        const { data } = await api.fetchUserStore(userId)
        dispatch({
            type: setSingleUserStore.type,
            payload: data
        })
        dispatch({ type: endLoading.type })
        return data
