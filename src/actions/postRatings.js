...
import { startLoading, endLoading, setPostRatings, setSinglePostRating, setUsersPostRatings, setUpdatePostRating, setDeletePostRating } from '../reducers/postRatingsSlice'
//action creators
...
export const actionFetchUserPostRatings = (postId, page) => async(dispatch) => {
    try{
        dispatch({ type: startLoading.type })
        const { data } = await api.fetchUserPostRatings(postId, page)
        dispatch({
            type: setUsersPostRatings.type,
            payload: data
        })
        dispatch({ type: endLoading.type })
        return data
